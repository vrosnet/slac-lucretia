...
char TrackThruVersion[] = "TrackThru Matlab version = 05-Oct-2005" ;
...
/* we need to allocate enough space to hold a vector of damping exponentials
   for each freqency-domain long-range wakefield */
		...
		if (nWakes[2] > 0)
		{
			TheBeam.TLRFreqDamping = 
				mxCalloc( nWakes[2] , sizeof(struct LucretiaComplex*) ) ;
			TheBeam.TLRFreqxPhase = 
				mxCalloc( nWakes[2] , sizeof(struct LucretiaComplex*) ) ;
			TheBeam.TLRFreqyPhase = 
				mxCalloc( nWakes[2] , sizeof(struct LucretiaComplex*) ) ;
		}
		if (nWakes[3] > 0)
		{
			TheBeam.TLRErrFreqDamping = 
				mxCalloc( nWakes[3] , sizeof(struct LucretiaComplex*) ) ;
			TheBeam.TLRErrFreqxPhase = 
				mxCalloc( nWakes[3] , sizeof(struct LucretiaComplex*) ) ;
			TheBeam.TLRErrFreqyPhase = 
				mxCalloc( nWakes[3] , sizeof(struct LucretiaComplex*) ) ;
		}
...
/* the number of bunches field in TheBeam is the total number of bunches,
	and the array of bunch pointers is allocated to this size: */
		...
		TheBeam.bunches = mxMalloc( TheBeam.nBunch * sizeof(struct Bunch*) ) ;
...
/* However, we only allocate space for the bunches which will actually be
   tracked on this call to TrackThru: */		
		...
		for (i=ArgStruc.FirstBunch-1 ; i < ArgStruc.LastBunch ; i++)
			TheBeam.bunches[i] = mxMalloc(sizeof(struct Bunch)) ;
...
*/
      ...
      TheBeam.bunches[i]->y = mxMalloc(6*npart*sizeof(double)) ;
			...
			if (nWakes[1] > 0)
			{
				TheBeam.bunches[i]->TSR = mxMalloc(nWakes[1]*sizeof(struct SRWF*)) ;
			...
			if (nWakes[2] > 0)
			{
				TheBeam.bunches[i]->TLRFreq = 
					mxMalloc(nWakes[2]*sizeof(struct LRWFFreq*)) ;
			...
			if (nWakes[3] > 0)
			{
				TheBeam.bunches[i]->TLRErrFreq = 
					mxMalloc(nWakes[3]*sizeof(struct LRWFFreq*)) ;
...
/* Now handle the case where data is to be returned.
   start by creating a structure array for BPM data */
	...
	bpmdat = mxCreateStructMatrix( 1, TrackArgs->nBPM, 8, BPMDatFieldName ) ;
	if (bpmdat == NULL)
	{
		status = 0 ;
		goto egress ;
...
/* now we do the instrument data, pretty similar to the BPMs but without 
   any optional fields and/or multi-dimensional arrays */
	...
	instdat = mxCreateStructMatrix( 1, TrackArgs->nINST, 9, INSTDatFieldName ) ;
	if (instdat == NULL)
	{
		status = 0 ;
		goto egress ;
...
/* now for the RF structure BPMs.  These are somewhat different in that
   each structure can have multiple BPMs, and the indexing of the data is by
	...
	sbpmdat = mxCreateStructMatrix( 1, TrackArgs->nSBPM, 4, SBPMDatFieldName ) ;
	if (instdat == NULL)
	{
		status = 0 ;
		goto egress ;
