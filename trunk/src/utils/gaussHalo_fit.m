function [yfit,q,dq,chisq_ndf] = gaussHalo_fit(x,y,dy,y_off)   
...
%
%    	  yfit =  q(1) + q(2)*exp( -(x-q(3))^2/2*q(4)^2 ) + q(3)*x^-q(4)
%    	  [x>q(5)] (in sigma units), for halo fit part
%
...
z = sqrt(2*pi)*p(2)*gauss(x,p(1),p(2)) + ...
  double((abs(x)./p(2))>p(5)).*p(3).*abs(x-p(1)).^-p(4);
if y_off
  Q = [ones(size(z)) z];
  [yfit,~,c] = fit(Q,y,dy);
  q = [c(1) c(2) p(1) p(2)];
