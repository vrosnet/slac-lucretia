function [q,dq,xf,yf] = gengauss_plot(x,y,dy,y_off,no_txt,ax,fcol,pcol)

%	[q,dq,xf,yf] = gengauss_plot(x,y[,dy,y_off,no_txt])
%
%	Fits and plots generalized gaussian curve data vectors "x" and "y" with
...
if ~exist('dy','var') || isempty(dy)
  [~,p,dp,chisq] = gengauss_fit(x,y);
%  yyf = p(1)*ones(size(xx)) + p(2)*sqrt(2*pi)*p(4)*gengauss(xx,p(3),p(4),p(5)); 
  yyf = p(1)*ones(size(xx)) + p(2)*(2*sqrt(gamma(1/p(5))/gamma(3/p(5)))*p(4)*gamma(1+1/p(5)))*gengauss(xx,p(3),p(4),p(5)); 
  ...
  [~,p,dp,chisq] = gengauss_fit(x,y,dy,y_off);
...
%  title('A + B*exp( -(X-C)^2/2*(D*(1+sign(X-C)*E))^2 )')
  title('A + B*exp(-abs((x-C)/(D*sqrt(gamma(1/E)/gamma(3/E))))^E)')
