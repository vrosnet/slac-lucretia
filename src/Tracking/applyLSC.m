...
%   dL = recommended minimum drift length for this bunch (0.1c/w_p)
...
if isequal(varargin{1},'plot')
  ...
  nbins=datastore(ind,bind).nbins;
  Y=datastore(ind,bind).Y;
  f=datastore(ind,bind).f;
  V=datastore(ind,bind).V;
  Z=datastore(ind,bind).Z;
  L=datastore(ind,bind).L;
  foffs=datastore(ind,bind).foffs;
  subplot(3,1,1), plot(tbins.*1e12,I_ns,'r'); 
  hold on
  plot(tbins.*1e12,I); hold off; grid
  legend({'Unsmoothed Current histogram' 'Smoothed data'});
  xlabel('t / ps'); ylabel('I / kA')
  subplot(3,1,2), semilogy(f,2*abs(Y(1:nbins/2+1))), grid
  hold on
  semilogy(f,2*abs(Z(1:nbins/2+1)),'r')
  xlabel('f / Hz')
  ylabel('PSD')
  legend({'I' 'Z'})
  ax=axis;
  line([foffs(1) foffs(1)],ax(3:4),'Color','k')
  line([foffs(2) foffs(2)],ax(3:4),'Color','k')
  hold off
  subplot(3,1,3), plot(tbins.*1e12,(real(V)/L).*1e-3), grid
  xlabel('t / ps')
  ylabel('V / keV.m^{-1}')
  return
end
...
cutoff=[0 0.9];
if isfield(tf,'LSC_cutoffFreq') && length(tf.cutoffFreq)==2
...
zbins=linspace(zmin,zmax,nbins);
[~,bininds] = histc(z,zbins);
